...
using StackExchange.RedisPlus.KeySpaceNotofications;
...
namespace StackExchange.RedisPlus
{
    public class RedisL1Database:IDatabase,IDisposable
    {
        ...
        public RedisL1Database(IDatabase redisDb,IMemoryCache cache)
            : this(string.Format("{0}:db={1}", redisDb.Multiplexer.Configuration, redisDb.Database), redisDb,new ObjMemCache(cache))
        {
        }
        ...
        private RedisL1Database(string uniqueId, IDatabase redisDb,IObjMemCache cache)
        {
            _uniqueId = uniqueId;
            ...
            _dbData = DatabaseRegister.Instance.GetDatabaseInstanceData(uniqueId, redisDb,cache);
        }
